...
float moisture;
int normalized_moisture;
static TimerHandle_t sensor_timer;

float app_driver_read_moisture() {
    // Example ADC reading
    float moisture = adc1_get_raw(MOISTURE_SENSOR_ADC_CHANNEL);
    normalized_moisture = (2200 - moisture) / (2200 - 850) * 100;
    normalized_moisture = fmaxf(0.0, fminf(normalized_moisture, 100.0));
    ESP_LOGI("MOSITURE_READ", "Normalised moisture: %i", normalized_moisture);
    ESP_LOGI("MOISTURE_READ", "Moisture level read: %f", moisture);
    if (normalized_moisture<50){
        printf("Soil is dry\n");
    }
    else {
        printf("Soil is moist\n");
    }
    return normalized_moisture;
}
...
void app_driver_set_water_pump(bool on) {
    ...
    if (on) {
        if (normalized_moisture < 50) { // Confirm that moisture levels still require pumping
...
static void app_sensor_update(TimerHandle_t handle)
{
    ...
    bool desired_state = (normalized_moisture< 50 );
    if (desired_state){
        ESP_LOGI("APP_SENSOR_UPDATE", "Water pump state changed to %s", desired_state ? "on" : "off");
        app_driver_set_water_pump(true);
    }
    else{
    ...
    esp_rmaker_param_update_and_report(
                ...
                esp_rmaker_float(normalized_moisture));
}
